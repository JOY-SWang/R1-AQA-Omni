...
from transformers import AutoProcessor, HfArgumentParser, Qwen2AudioForConditionalGeneration, Qwen2_5OmniThinkerForConditionalGeneration, Qwen2_5OmniForConditionalGeneration, Qwen2_5OmniProcessor
...
def main():
    ...
    # audio_processor = Qwen2_5OmniProcessor.from_pretrained(data_args.model_path, padding_side="left")
    # audio_processor.pad_token_id = audio_processor.tokenizer.pad_token_id
    # audio_processor.eos_token_id = audio_processor.tokenizer.eos_token_id
    audio_model = Qwen2_5OmniThinkerForConditionalGeneration.from_pretrained(data_args.model_path, torch_dtype=torch.bfloat16, device_map="auto")
    ...
    for i in tqdm(range(0, len(datas), batch_size)):
        ...
        inputs = audio_processor(
            text=text, audio=batch_audios, sampling_rate=16000, return_tensors="pt", padding=True).to(audio_model.device)

        generated_ids = audio_model.generate(**inputs, max_new_tokens=120)
