...
from transformers import (
    ...
    Qwen2_5OmniThinkerForConditionalGeneration, Qwen2_5OmniConfig, 
    # Qwen2_5OmniModel, 
...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            elif "Qwen2___5-Omni" in model_id:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        prompts = [x["prompt"][0]["content"][1]["text"] for x in inputs]
        ...
        prompt_inputs = prompt_inputs.to(model.device).to(model.dtype)
        ...
        # Prepare input features and masks if present (for Qwen2.5-omni, may not be needed, but check for audio features)
        prompt_ids, prompt_mask = prompt_inputs["input_ids"], prompt_inputs["attention_mask"]
        
        features_values = prompt_inputs.get("input_features", None)
        features_masks = prompt_inputs.get("feature_attention_mask", None)
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            if hasattr(prompt_completion_ids, "sequences"):
                prompt_completion_ids = prompt_completion_ids.sequences
            ...
            prompt_ids = prompt_completion_ids[:, :prompt_inputs["input_ids"].shape[1]]
            completion_ids = prompt_completion_ids[:, prompt_inputs["input_ids"].shape[1] :]
        ...
        # Mask everything after the first EOS token in completions
        ...
        with torch.inference_mode():
            if self.ref_model is not None:
                ref_per_token_logps = self._get_per_token_logps(
                    self.ref_model, prompt_completion_ids, attention_mask, features_values, features_masks
                )
            else:
        ...
        # Compute the rewards
        prompts_expanded = [prompt for prompt in prompts for _ in range(self.num_generations)]
        ...
        # print(rewards_per_func)
        # Change the format reward
        # tensor([[1.0000, 0.3000],[0.0000, 0.3000]], device='cuda:4')

        # if rewards_per_func.shape[1] >= 2:
        #     format_reward_vals = rewards_per_func[:, 1]
        #     accuracy_reward_vals = rewards_per_func[:, 0]
        #     if format_reward_vals.mean().item() < 0.7:
